...
/*   Updated: 2025/05/31 12:19:05 by tat-nguy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
	...
	philo->max_meals = -1;
	philo->philo_enough = -1;
	if (ac == 6)
	{
		philo->max_meals = ft_atol(av[5]);
		philo->philo_enough = 0;
	}
	...
	// set up philo->pid
	// philo->pid = (int *)malloc(sizeof(int) * philo->num_philos);
	...
	sem_unlink("/forks");
	philo->forks = sem_open("/forks", O_CREAT | O_EXCL, 0644, philo->num_philos);
	
	if (!philo->write_lock || !philo->forks)
